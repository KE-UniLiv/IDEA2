...
def configure_prompt(role: str = config["role"],
                     ...
                     ignore_schemas: bool = False, 
                     useOntology=False, ontology_path=None, cqs=None) -> str:
    """
    ...
    ontology = ontology_enrichment.load_ontology(ontology_path=ontology_path) if useOntology else None

    prompt = prompt_builder.get_prompt(
        raw_data=combined_schemas if not ignore_schemas and not useOntology else ontology if useOntology else "Error: loaded no source ontology or schemas!",
        include_role=False,
    )

    return prompt, combined_schemas if not useOntology else ontology


def run_cq_extraction(model: object, prompt, enrichment=False) -> list:
    """
    ...
    Args:
        ...
        enrichment (bool): Whether to enrich the ontologies with CQs (Applies pydantic constraints).
    ...
    response = model.generate(prompt, enriched=enrichment)
    if not response:
        raise ValueError("No response received from the model. Please check the model and prompt.")
